...
#
# The role trusts the cluster OIDC provider (created in main.tf) for the
# cilium-operator ServiceAccount in kube-system; the Helm release (cilium.tf)
# annotates that SA with the role ARN below.

locals {
  operator_namespace       = "kube-system"
  operator_service_account = "cilium-operator"

  # OIDC condition keys use the issuer host without the scheme. The provider's
  # url attribute may or may not carry https:// depending on provider version,
  # so strip it defensively.
  cilium_oidc_url_no_scheme = replace(aws_iam_openid_connect_provider.cluster.url, "https://", "")
}

resource "aws_iam_role" "cilium_operator" {
  name = "${local.cluster_name}-cilium-operator"
  ...
  assume_role_policy = jsonencode({
    ...
    Statement = [{
      ...
      Condition = {
        StringEquals = {
          "${local.cilium_oidc_url_no_scheme}:aud" = "sts.amazonaws.com"
          "${local.cilium_oidc_url_no_scheme}:sub" = "system:serviceaccount:${local.operator_namespace}:${local.operator_service_account}"
        }
      }
    }]
  })
