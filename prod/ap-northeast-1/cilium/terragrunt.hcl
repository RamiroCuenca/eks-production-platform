...
dependency "eks" {
  ...
  mock_outputs = {
    cluster_name     = "eks-platform-prod-ap-northeast-1"
...
inputs = {
  ...
  # Prod posture: run the operator HA (two replicas) since the ENI allocator is
  # on the path for every new node coming up.
