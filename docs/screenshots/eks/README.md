...
| `iam-karpenter-controller-role.png` | Karpenter controller role → Trust relationships: federated trust resolves to the cluster OIDC provider with `sub = system:serviceaccount:karpenter:karpenter` and `aud = sts.amazonaws.com`. Proves IRSA wiring is correct before the gitops repo deploys the actual controller. |
